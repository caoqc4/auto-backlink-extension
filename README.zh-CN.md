...
1. 项目资料
   维护品牌名、网站 URL、描述、关键词、联系方式等基础信息。

2. 发现 / 导入
   从 Ahrefs、Semrush、表格文件或当前网页收集潜在外链机会。

3. 资源池
   按根域名去重，识别资源类型，记录优先级和候选页面。

4. 页面预检测
   检测表单、登录/注册门槛、验证码、已有链接和 rel 属性。

5. 半自动执行
   生成草稿，辅助填写可识别字段，最终提交由用户确认。

6. 追踪 + 同步
   记录状态，检查结果，并可选同步到 Google Sheets 备份。
```

插件围绕一条可重复执行的工作流组织：维护项目资料，收集外链机会，预检测页面，辅助执行，检查结果，再把过程记录同步出去。Google Sheets 不是必需项，但建议作为备份、复盘和跨设备处理层。
...
外部服务
├─ Google Sheets：可选备份与恢复
├─ AI 服务商：用户自己的 Key，用于生成草稿
└─ Ahrefs / Semrush：外链来源发现
```

整体上，Popup / 侧边栏是操作台，Content Script 负责分析和辅助填写当前网页，Service Worker 负责后台任务调度，IndexedDB 是本地数据源。外部服务都由用户自己控制：AI 使用用户自己的 API Key，Google Sheets 只同步到用户配置的表格。
